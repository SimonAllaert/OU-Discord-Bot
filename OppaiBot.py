...
async def on_raw_reaction_add(payload):
	if payload.channel_id == 570939014329401345 and not bot.get_user(payload.user_id).bot:
...
async def room(ctx):
	if discord.utils.get(ctx.guild.voice_channels, name = ctx.author.name + "'s room"):
		...
		channel = await ctx.guild.create_voice_channel(ctx.author.name + "'s room", category = discord.utils.get(ctx.guild.categories, id = 580166156095062027))
